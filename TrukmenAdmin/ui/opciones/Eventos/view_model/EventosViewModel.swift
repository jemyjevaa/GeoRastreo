...
class EventosViewModel {
    private let service: EventosServiceProtocol
...
var didGetEvent: (([ItemEvents]) -> ())?
var didGetPosition: ((PositionElement) -> ())?
var fecha1:Date? = nil
var fecha2:Date? = nil
    
    init(withPromociones serviceProtocol: EventosServiceProtocol = EventosService()){
    ...
    func EventsService(Authorization: String, deviceId: Int, type: String, from: String, to: String ){
             //  print("RUTAS SERVICE ")
               switch networkStatus {
               case .offline:
                   self.isDisconnected = true
               ...
               case .online:
                   // call your service here
                   self.service.serviceEvents(Authorization: Authorization, deviceId: deviceId, type: type, from: from, to: to, success: { (data) in
                       self.isLoading = false
                     //  self.model = data
                     //  self.count = data.datos.count
                       self.didGetEvent?(data)
    ...
    func positionService(Authorization: String, id: Int){
        //  print("RUTAS SERVICE ")
                      switch networkStatus {
                      case .offline:
                          self.isDisconnected = true
                      ...
                      case .online:
                          // call your service here
                          self.service.servicePosition(Authorization: Authorization, id: id, success: { (data) in
                              self.isLoading = false
                            //  self.model = data
                            //  self.count = data.datos.count
                              self.didGetPosition?(data)
