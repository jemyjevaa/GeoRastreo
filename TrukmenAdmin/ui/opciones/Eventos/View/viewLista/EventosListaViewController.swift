...
import MarqueeLabel
import Toast
class EventosListaViewController: UIViewController{
    ...
    var eventsData = [ItemEvents]()
    var eventsFilter = [ItemEvents]()
    var marcadores : Bool?
    var titulo = ""
    var subtitulo = ""
    var unity : ItemUnidades!
    static var vc = EventosListaViewController()
    ...
    func CargaDatosInicial(){
        self.subtitleReporte.text = self.unity.name!
    }
}

extension EventosListaViewController: UITableViewDelegate, UITableViewDataSource, UISearchBarDelegate{
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let cell = Bundle.main.loadNibNamed("CellsListaReporteEventos", owner: self, options: nil)?.first as! CellsListaReporteEventos
      
        
        var f1 =  eventsData[indexPath.row].eventTime!.split(separator: ".")
        print("Nombre evento ", eventsData[indexPath.row].type!)
        cell.eventName.text = IdiomaComandos().seleccionIdiomaComandos(caso: eventsData[indexPath.row].type!)
        cell.horaName.text = "Hora : " + convertDateFormatter(date: String(f1[0]))
        cell.unidadName.text = "Unidad: " + unity.name!
        return cell
    }
    ...
    func tableView(_ tableView: UITableView, heightForRowAt indexPath: IndexPath) -> CGFloat {
        ...
        func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
            ...
            let cell = tableView.cellForRow(at: indexPath) as! CellsListaReporteEventos
            let img =   #imageLiteral(resourceName: "infoposicion")
            let screenRect = UIScreen.main.bounds
            let screenHeight = screenRect.size.height - 100.0
            ...
            /*   cell.card.backgroundColor = UIColor(hexString: "#e6\((    FuncionesGlobales().deleteCodeColor2(code2:(UserManager.currentUser?.datosEmpresa.color2)!)))")
               ...
               */
               DispatchQueue.main.asyncAfter(deadline: .now() + 0.3) {
                   // your code here
                   if(self.eventsData[indexPath.row].positionId != 0){
                       let vcn = UIStoryboard(name: "EventoViewMapa", bundle: nil)
                                        .instantiateViewController(withIdentifier: "EventoViewMapa") as! EventoViewMapa
                       vcn.delegate = self.delegate
                       vcn.viewModel = self.viewModel
                       vcn.eventData = self.eventsData[indexPath.row]
                       vcn.unidadName = self.unity.name!
                                    self.navigationController?.pushViewController(vcn,animated: true)
                   }else {
                    print("Sin informacion ")
                      // self.view.makeToast("Evento sin ubicación")
                       
                       self.view.makeToast("El evento seleccionado no tiene información de ubicación", duration:5, point: CGPoint(x: screenWidth, y: screenHeight), title: "Evento sin ubicación", image: img, completion: { didTap in
